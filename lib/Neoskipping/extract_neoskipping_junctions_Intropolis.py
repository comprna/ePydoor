...
extract_neoskipping_junctions_Intropolis: identify the junctions that could generate an neoskipping
exon in the Intropolis dataset
...
parser.add_argument("-i", "--input", required=True,
                    help="Input file")
parser.add_argument("-I", "--input_Introplis", required=True,
                    help="Input file Intropolis")
...
def extract_neoskipping_junctions_Intropolis(input_path_George, input_path_Intropolis, gtf_path, threshold, output_path):

    try:
        logger.info("Starting execution")

        # 1. Load the readCounts from Intropolis
        junction_reads_Intropolis = {}
        cont = 0
        with open(input_path_Intropolis) as f:
            logger.info("Loading readCounts Intropolis...")
            for line in f:
                cont += 1
                # print(str(cont))
                # if (cont == 100):
                ...
                junction_id = tokens[0]+";"+str(int(tokens[1])-2)+";"+str(int(tokens[2])+1)+";"+str(tokens[3])
                ...
                if(junction_id not in junction_reads_Intropolis):
                    junction_reads_Intropolis[junction_id] = reads
                else:
                    logger.info("Junction id "+junction_id+" repeated")
                    pass

        # 2. Load the gtf file : Gene -> associated_exons
        ...
        # 3. Load the readcounts from George et al. Save all the junctions (type 2) to each associated gene: Gene -> junctions_type==2
        gene_junctions_1, gene_junctions_2, junction_type_dict = {}, {}, {}
        cont = 0
        with open(input_path_George) as f:
            ...
            for line in f:
                ...
                #Save also the reads associated --> Not necesary to save the reads in this script
                # reads = list(map(int,tokens[8:]))
                for x in gene_list:
                    ...
                    if(junction_type=="1"):
                        if(x in gene_exons):
                            ...
                            # Save this reads in the dictionary --> Not necesary to save the reads in this script
        ...
        # 4. Go over the junctions lists and the exons and check if there is neoskipping
        ...
        output_file.write("Sample_id\tGene\tNeoskipping_junction\tSkipped_junctions\tNeoskipping_ReadCounts\tmean_skipped_ReadCounts\n")
        cont = 0
        for neoskipping_junction,skipped_junctions in junctions_inside.items():
            ...
            junction_aux = ";".join(neoskipping_junction.split(";")[1:])
            # Get the reads associated to the junction in Intropolis
            if(junction_aux in junction_reads_Intropolis):
                neoskipped_read = junction_reads_Intropolis[junction_aux]
                # If the reads is above the threshold, output this neoskipping
                if (neoskipped_read >= threshold):
                    #Look also for the reads in the central junctions
                    # Get also the reads associated to the skipped junctions for this sample
                    total_junctions, total_reads = [], []
                    mean_reads = 0
                    for x in skipped_junctions:
                        junction_aux2 = ";".join(x.split(";")[1:])
                        total_junctions.append(junction_aux2)
                        if (junction_aux2 in junction_reads_Intropolis):
                    ...
                    #Output the values
                    sample_id = "Intropolis"
                    output_file.write(sample_id+"\t"+gene+"\t"+junction_aux+"\t"+
